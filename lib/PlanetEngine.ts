import { Engine, Scene, Vector3, FreeCamera, MeshBuilder, StandardMaterial, Texture, Color4, Color3, MultiMaterial, RenderTargetTexture, DynamicTexture, Animation, CubicEase, EasingFunction, Quaternion, Mesh, Tools, ArcRotateCamera, SubMesh, HemisphericLight, Camera, PointLight, VertexData, Matrix } from '@babylonjs/core';
...
export class PlanetEngine {
    ...
    async processZoomImages(pickedPoint: Vector3) {
        ...
        try {
            ...
            for (let i = 0; i < numLayers; i++) {
                ...
                canvas.height = 1024;
                const ctx = canvas.getContext('2d')!;
                // Crop center 25% (50% width × 50% height = 2x zoom)
                ctx.drawImage(img, 256, 256, 512, 512, 0, 0, 1024, 1024);
    ...
    async approach(onComplete?: () => void) {
        ...
        const approachObserver = this.scene.onBeforeRenderObservable.add(() => {
            ...
            this.arcCamera.alpha = startAlpha + (targetAlpha - startAlpha) * angularEase;
            this.arcCamera.beta = startBeta + (targetBeta - startBeta) * angularEase;
